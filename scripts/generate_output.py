...
state_dict = torch.load('../model_checkpoints/histogpt-1b-6k-pruned.pth?download=true', map_location=device)
histogpt.load_state_dict(state_dict, strict=True)
tokenizer = BioGptTokenizer.from_pretrained("microsoft/biogpt")

# Directory with h5 feature files
features_dir = '../competition_data/REG_sample_processed/h5_files/256px_ctranspath_0.0mpp_4.0xdown_normal/'
h5_files = sorted(glob.glob(os.path.join(features_dir, "*.h5")))

# PROMPT
prompt_text = 'Extract Organ and Cancer type and give full diagnosis'
...
with open(os.path.join(output_dir, "batch_reports_sample.json"), "w") as f:
